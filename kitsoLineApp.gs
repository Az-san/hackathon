...
const channelAccessToken = "KCjhNj+MdQsmmr2G+hkMz+iwrWYv6abSzgKQ02DppECxEX2B2SB7dE1kSGSUKtqd/IcJse+slJ9AWlMZV3scMLchMna3PCqhfYg0MqLLfhHrb15y+iVAaBmchnvIpg27glTQ3k0U775+n7WWie374QdB04t89/1O/w1cDnyilFU=";
...
function doRemind()
{
  ...
  if(endRow != 0){
    ...
    {
      // 今週分のリマインダーを流したいな
      var broadcastMessage = "今日はやすもう。。。（訳：リマインドすることはないよ）"
    }
    sheet_Log.appendRow([broadcastMessage]);
...
function sendSchedule(){
  ...
  {
    var broadcastMessage = getMessage(0);
    broadcast(broadcastMessage)
  }
  return 0;
}
...
function contdown(event)
{
  ...
  const t  = Date.now() - nt;
  const n = 307 - Math.floor( t / 1000 / 60 / 60 / 24 ) + 1;// 11/4 - 今日
  if(n>0 && n!=0){
...
{
  const gptKey  = "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";
